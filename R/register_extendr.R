...
register_extendr <- function(path = ".", quiet = FALSE, force = FALSE, compile = NA) {
  ...
  # If the wrapper file is newer than the DLL file, assume assume it's been
  # generated by the latest DLL, which should mean it doesn't need to be
  # regenerated. This isn't always the case (e.g. when the user accidentally
  # edited the wrapper file by hand) so the user might need to run with
  # `force = TRUE`.
  if (!isTRUE(force) &&
