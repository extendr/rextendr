...
register_extendr <- function(path = ".", quiet = FALSE, force_wrappers = FALSE) {
  ...
  success <- fish_wrappers(src_dir, outfile, quiet = quiet)
...
fish_wrappers <- function(src_dir, outfile, quiet = FALSE) {
  ...
  exit_code <- system2("make", c("-C", src_dir, "-f", makevars_file, "extendr-wrappers.R"))
  if (exit_code != 0L) {
    warning(
      glue("Re-generating wrappers failed. Does src/{makevars_file} have `extendr-wrappers.R` target?"),
      call. = FALSE
    )
  }
