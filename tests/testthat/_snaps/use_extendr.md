...
---
    ...
    Output
      ...
      PKG_LIBS = -L$(CURDIR)/$(LIBDIR) -ltestpkg
      DOCUMENT_SHLIB = rust/document.so
      ...
      $(STATLIB):
      	...
      	# Generate R wrappers: cd rust/ so R CMD SHLIB doesn't see THIS Makevars and recurse.
      	cd rust && "$(R_HOME)/bin/R" CMD SHLIB -o document.so document.c $(PKG_LIBS)
      	"$(R_HOME)/bin/Rscript" rust/document.R
      	rm -f $(DOCUMENT_SHLIB) $(DOCUMENT_OBJ)
      ...
      clean:
      	rm -Rf $(SHLIB) $(STATLIB) $(OBJECTS) $(TARGET_DIR) $(VENDOR_DIR) $(DOCUMENT_SHLIB) $(DOCUMENT_OBJ)
...
---
    ...
    Output
      ...
      STATLIB = $(LIBDIR)/libtestpkg.a
      PKG_LIBS = -L$(CURDIR)/$(LIBDIR) -ltestpkg -lws2_32 -ladvapi32 -luserenv -lbcrypt -lntdll
      DOCUMENT_SHLIB = rust/document.dll
      DOCUMENT_OBJ = rust/document.o
      ...
      $(STATLIB):
        ...
        # vendor.tar.xz exists, it is unzipped and used for offline compilation.
      	...
      	# Generate R wrappers: cd rust/ so R CMD SHLIB doesn't see THIS Makevars.win and recurse.
      	cd rust && "$(R_HOME)/bin$(R_ARCH_BIN)/R" CMD SHLIB -o document.dll document.c $(PKG_LIBS)
      	"$(R_HOME)/bin$(R_ARCH_BIN)/Rscript" rust/document.R
      	rm -f $(DOCUMENT_SHLIB) $(DOCUMENT_OBJ)
...
---
    ...
    Output
      ...
      extern SEXP wrap__make_testpkg_wrappers(SEXP use_symbols, SEXP package_name);
      
      static const R_CallMethodDef CallEntries[] = {
          {"write_wrappers", (DL_FUNC) &wrap__make_testpkg_wrappers, 2},
          {NULL, NULL, 0}
      };
...
---
    ...
    Output
      ...
      cat(
        ...
        "#' @useDynLib testpkg, .registration = TRUE\n",
        "NULL\n\n",
        .Call("write_wrappers", TRUE, "testpkg"),
        "# nolint end\n",
        sep = "",
...
# use_extendr() skip pre-existing files in non-interactive sessions
    ...
    Message
      ...
      > File 'src/rust/document.c' already exists. Skip writing the file.
      > File 'src/rust/document.R' already exists. Skip writing the file.
...
# use_extendr() can overwrite files in non-interactive sessions
    ...
    Message
      ...
      v Writing 'src/rust/document.c'
      v Writing 'src/rust/document.R'
      > File 'R/extendr-wrappers.R' already exists. Skip writing the file.
