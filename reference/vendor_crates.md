...
`vendor_crates()` is used to package the dependencies as required by
CRAN. It executes `cargo vendor` on your behalf creating a `vendor/`
directory and a compressed `vendor.tar.xz` which will be shipped with
package itself. If you have modified your dependencies, you will need
need to repackage the vendored dependencies using `vendor_crates()`.
...
- `vendor_crates()` returns a data.frame with two columns `crate` and
  `version`
