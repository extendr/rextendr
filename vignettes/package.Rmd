...
One thing we want to emphasize here is that these functions are needed solely for
package development. The package doesn't depend on the rextendr package, just like that
a package never adds the devtools package to `Depends` or `Imports` no matter how often
...
Now we are just one step away (as the message says, we need to run `rextendr::document()`) from calling Rust functions from R.
...
* **`src/Makevars`**, **`src/Makevars.win`**: These hook `cargo build` at the installation of the R package. In most cases, we don't edit these.
...
As all things are done by `rexetndr::document()` already, we can just load the package 
...
generated from the above Rust code. Let's look at `R/extendr-wrappers.R`:
